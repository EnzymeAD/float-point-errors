...
int reorderMesh(Mesh *m) 
{
  ...
  v2   = (double *)malloc(sizeof(double)*2*n);
...
int setVertices(double *verts, int nv, Mesh *m)
{
  ...
  memcpy(m->d->v, verts, 2*sizeof(double)*nv);
...
#else
  memcpy(tv, hv, 2*sizeof(double)*n);
#endif
...
int getVertices(double *verts, int nv, Mesh *m)
{
  ...
  memcpy(verts, m->d->v, 2*sizeof(double)*nv);
  return 0;
}
