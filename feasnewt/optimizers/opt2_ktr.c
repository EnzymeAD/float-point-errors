...
static void matstr(int *hrow, int *hcol, const Mesh *mesh)
{
  ...
  for (i = 0; i < nn; ++i) {
    ...
    hrow[0] = 2*i+0; hcol[0] = c+0;
    hrow[1] = 2*i+0; hcol[1] = c+1;
    hrow[2] = 2*i+1; hcol[2] = c+1;
    hrow += 3; hcol += 3;
    ...
    for (j = 1; j < l; ++j) {
      ...
      hrow[0] = 2*i+0; hcol[0] = c+0;
      hrow[1] = 2*i+0; hcol[1] = c+1;
      hrow[2] = 2*i+1; hcol[2] = c+0;
      hrow[3] = 2*i+1; hcol[3] = c+1;
      hrow += 4; hcol += 4;
    }
  }
  return;
...
int optMesh(Mesh *mesh, int max_iter, double cn_tol, int precond)
{
  ...
  double *hv_in, *hv_out;       /* Hessian vector product */
  ...
  hv_in = (double *)malloc(nvar*sizeof(double));
  hv_out = (double *)malloc(nvar*sizeof(double));
  ...
  KTR_set_int_param(kc, KTR_PARAM_HESSOPT, KTR_HESSOPT_PRODUCT);
  ...
  do {
    ...
    case KTR_RC_EVALGA:
      ferr = gFcn(&objn, mesh);
      needH = 1;		/* EVALGA only called after step accepted */
      break;
	...
	needH = 0;
      ...
      memset(hv_out, 0, nvar*sizeof(double));
      matmul(hv_out, mesh, hv_in);
      memcpy(hv_in, hv_out, nvar*sizeof(double));
      break;
    }
    ...
    status = KTR_solve(kc, &obj, 0, 
		       ...
		       nnzh, NULL, NULL, NULL, hv_in, NULL);
  } while (status > 0);
