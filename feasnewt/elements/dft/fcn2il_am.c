...
static int g_fcnl(double *obj, double g_obj[2], 
		  const double x[6], const double w[4], const double t[4])
{
  ...
  *obj = f * loc1;
  ...
  g_obj[0] = w[3]*(f*matd[1] - g*matr[2]);
  g_obj[1] = w[3]*(f*matd[3] + g*matr[0]);
  return 0;
}
		  ...
		  const double x[6], const double w[4], const double t[4])
{
  ...
  *obj = f * loc1;
  ...
  g_obj[0] = w[3]*(dobj_df*matd[1] - dobj_dg*matr[2]);
  g_obj[1] = w[3]*(dobj_df*matd[3] + dobj_dg*matr[0]);
  ...
  h_obj[0] = t1*(dobj_df + dg[0]*(matd[1] + loc1));
  h_obj[1] = t1*(dg[0]*matd[3] + loc1*dg[1]);

  h_obj[2] = t1*(dobj_df + dg[1]*(2.0*matd[3] + dobj_dgdg*dg[1]));
  return 0;
}
#endif
...
int g_fcnl_2(double *obj, double g_obj[2], const double x[6])
{
  ...
  return 0;
#else
  return g_fcnl(obj, g_obj, x, w2, t2);
#endif
}
...
int h_fcnl_2(double *obj, double g_obj[2], double h_obj[3], const double x[6])
{
  ...
  return 0;
#else
  return h_fcnl(obj, g_obj, h_obj, x, w2, t2);
#endif
}
