	     ...
	     const double w[6], const double t[9])
{
  ...
  *obj = f * loc4;
  ...
  g_obj[0] = w[5]*(f*matd[2] + g*loc3);
  g_obj[1] = w[5]*(f*matd[5] + loc2*matr[6] - loc1*matr[7]);
  g_obj[2] = w[5]*(f*matd[8] + loc1*matr[4] - loc2*matr[3]);
  return 0;
}
	     ...
	     const double w[6], const double t[9])
{
  ...
  *obj = f * loc1;
  ...
  /* Calculate the derivative of the objective function. */
  g_obj[0] = w[5]*(dobj_df*matd[2] + dobj_dg*dg[2]);
  g_obj[1] = w[5]*(dobj_df*matd[5] + dobj_dg*dg[5]);
  g_obj[2] = w[5]*(dobj_df*matd[8] + dobj_dg*dg[8]);
  ...
  /* Blocks for the Hessian construction */
  loc1 = dobj_dgdg*dg[2] + matd[2];
  h_obj[0] = t1*(dobj_df + dg[2]*(matd[2] + loc1));
  h_obj[1] = t1*(dg[2]*matd[5] + loc1*dg[5]);
  h_obj[2] = t1*(dg[2]*matd[8] + loc1*dg[8]);

  loc1 = dobj_dgdg*dg[5] + matd[5];
  h_obj[3] = t1*(dobj_df + dg[5]*(matd[5] + loc1));
  h_obj[4] = t1*(dg[5]*matd[8] + loc1*dg[8]);

  h_obj[5] = t1*(dobj_df + dg[8]*(2.0*matd[8] + dobj_dgdg*dg[8]));
  return 0;
}
