...
/*                                                                           */
/* The code requires 440 flops.  The gradient evaluation needs 130 flops     */
...
static void assemble_diag(double h_obj[10], const double J_A[6])
{
  ...
  loc1 = J_A[0] + J_A[3] + 2.0*J_A[4] + J_A[5];
  loc2 = 2.0*(J_A[1] + J_A[2]);

  h_obj[0] = loc1 + loc2;

  h_obj[1] = loc1 - loc2;

  h_obj[2] = 4.0*(J_A[3] - J_A[4]) + J_A[5];
...
static void assemble_offdiag(double h_obj[16], const double J_B[9])
{
  ...
  loc1 = J_B[0] + J_B[4] + J_B[5] + J_B[7] + J_B[8];
  loc2 = J_B[1] + J_B[2] + J_B[3] + J_B[6];

  h_obj[0] = loc1 + loc2;

  h_obj[1] = loc1 - loc2;

  h_obj[2] = 4.0*J_B[4] - 2.0*(J_B[5] + J_B[7]) + J_B[8];
...
int h_fcn(double *obj, double g_obj[12], double h_obj[24], const double x[12])
{
  ...
  *obj = f * loc1;
  ...
  assemble_diag(h_obj + 12, J_D);
  assemble_diag(h_obj + 20, J_F);

  assemble_offdiag(h_obj + 4, J_B);
  assemble_offdiag(h_obj + 8, J_C);
  assemble_offdiag(h_obj + 16, J_E);
  return 0;
}
