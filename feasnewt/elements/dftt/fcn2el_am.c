...
static int g_fcnl(double *obj, double g_obj[2], const double x[6])
{
  ...
  *obj = f * loc1;
  ...
  g_obj[0] = tsqrt3*(f*df[0] - g*matr[2]);
  g_obj[1] = tsqrt3*(f*df[1] + g*matr[0]);
  return 0;
}
		  ...
		  const double x[6])
{
  ...
  *obj = f * loc1;
  ...
  g_obj[0] = tsqrt3*(dobj_df*df[0] + dobj_dg*dg[0]);
  g_obj[1] = tsqrt3*(dobj_df*df[1] + dobj_dg*dg[1]);
  ...
  h_obj[0] = fthirds*(loc1*df[0] + loc2*dg[0] + dobj_df*(fmat[0]+t1+aux[0]));
  h_obj[1] = fthirds*(loc1*df[1] + loc2*dg[1] + dobj_df*(fmat[1] + aux[1]));

  loc1 = dg[1]*dobj_dfdg; 
  loc2 = dg[1]*dobj_dgdg + df[1]*dobj_dfdg;
  h_obj[2] = fthirds*(loc1*df[1] + loc2*dg[1] + dobj_df*(fmat[2]+t1+aux[2]));
  return 0;
}
#endif
