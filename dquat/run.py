...
FPOPTFLAGS_BASE = [
    ...
    "--enzyme-print-fpopt",
    "-mllvm",
    "--fpopt-log-path=dquat.txt",
    "-mllvm",
    "--fpopt-target-func-regex=dquat_demap_T",
    "-mllvm",
    "--fpopt-enable-solver",
    "-mllvm",
    "--fpopt-enable-pt",
...
def compile_fpopt(budget):
    ...
    command = [CXX] + CXXFLAGS + fpopt_flags + ["dquat.cpp", "-o", os.path.join(OUTPUT_DIR, f"dquat-fpopt-{budget}.exe")]
...
def main():
    ...
    print(f"Starting compilation of dquat-fpopt.exe with {len(budgets)} budgets using {num_workers} parallel workers.")
